...
public:									// We use smart pointers to identify the objects
	...
	std::list<std::shared_ptr<Hormone>> get_pheromone_list();
	std::vector<std::vector<std::shared_ptr<Hormone>>> get_pheromone_matrix();
...
private:
	...
	std::list<std::shared_ptr<Hormone>> _pheromone_list;
	//Pheromone position matrix :
	std::vector<std::vector<std::shared_ptr<Hormone>>> _pheromone_matrix ;
};

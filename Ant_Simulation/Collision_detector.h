...
// class to calculate the interactions of the items in the simulation
...
public:							//It is build thanks to a Table_of_items and three
								//doubles for the max radius
	Collision_detector(std::shared_ptr<Table_of_items>, double, double, double);
	...
	//Get functions :
	//get a list of all ants in collision with our ant
	std::list<std::shared_ptr<Ant>> get_an_coll(std::shared_ptr<Ant>);
	//get a list of all colonies in collision with our ant
	std::list<std::shared_ptr<Colony>> get_co_coll(std::shared_ptr<Ant>);
	//get a list of all foods in collision with our ant
	std::list<std::shared_ptr<Food>> get_fo_coll(std::shared_ptr<Ant>);
	//get a list of all pheromones in collision with our ant
	std::list<std::shared_ptr<Pheromone>> get_ph_coll(std::shared_ptr<Ant>);
